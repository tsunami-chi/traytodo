...
# VVVVVV (ИЗМЕНЕНИЕ 1: Добавляем QStandardPaths) VVVVVV
...
class TaskWidget(QtWidgets.QWidget):
    ...
    def __init__(self, task_data, font, main_app, parent=None):
        super().__init__(parent)
        self.task_data = task_data 
        self.main_font = font
        self.main_app = main_app 
        self.uid = task_data.get("uid")
    ...
    def on_check_changed(self, state):
        ...
        success = self.main_app.update_task_in_file(self.uid, "checked", is_checked)
        
        if success:
            self.task_data["checked"] = is_checked
            self.update_style()
        else:
            self.check_box.stateChanged.disconnect(self.on_check_changed)
            self.check_box.setChecked(not is_checked)
            self.check_box.stateChanged.connect(self.on_check_changed)
    ...
    def show_task_menu(self, position):
        ...
        if action == delete_action:
            self.main_app.delete_task_from_file(self.uid)
        elif action == edit_action:
            self.on_edit_task()
        elif action == important_action:
            is_important = important_action.isChecked()
            success = self.main_app.update_task_in_file(self.uid, "important", is_important)
            if success:
                self.task_data["important"] = is_important
                self.update_style()
            else:
                important_action.setChecked(not is_important)
    ...
    def on_save_edit(self):
        ...
        if new_text and new_text != current_text:
            success = self.main_app.update_task_in_file(self.uid, "text", new_text)
            if success:
...
class ToDoApp(QtWidgets.QWidget):
    def __init__(self):
        ...
        # VVVVVV (ИЗМЕНЕНИЕ 2: Меняем логику путей) VVVVVV
        if getattr(sys, 'frozen', False):
            # Ресурсы (иконка, about) лежат ВНУТРИ .exe
            self.internal_dir = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
        else:
            # Ресурсы лежат рядом с .py
            self.internal_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Данные (tasks.json) лежат в AppData
        self.external_dir = QStandardPaths.writableLocation(QStandardPaths.StandardLocation.AppDataLocation)
        
        # Создаем папку AppData, если ее нет
        if not os.path.exists(self.external_dir):
            try:
                os.makedirs(self.external_dir, exist_ok=True)
            except Exception as e:
                print(f"Could not create AppData directory: {e}")
                # Аварийный режим: пишем рядом с .exe (может не работать)
                self.external_dir = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.path.dirname(os.path.abspath(__file__))
        ...
        print(f"--- DATA FILE LOCATION: {self.TASKS_FILE} ---")
        # ^^^^^^^^ КОНЕЦ ИЗМЕНЕНИЯ 2 ^^^^^^^^
        
        self.initUI()
        self.createTrayIcon()
        
        self.load_and_redraw_all()
    ...
    def read_tasks_from_file(self):
        """Читает tasks.json и возвращает СТРУКТУРУ данных."""
        if not os.path.exists(self.TASKS_FILE):
            return {"group_by_period": True, "tasks": []}
        ...
        try:
            ...
            if "tasks" not in data:
                data["tasks"] = []
            if "group_by_period" not in data:
                 data["group_by_period"] = True
            
            return data

        except (json.JSONDecodeError, UnicodeDecodeError):
            print("Error reading tasks.json, resetting.")
            return {"group_by_period": True, "tasks": []}
            
    def write_data_to_file(self, data):
        """Перезаписывает tasks.json данной СТРУКТУРОЙ."""
        try:
            ...
            return False # Сохранить не удалось

    def load_and_redraw_all(self):
        """
        Главная функция. Читает файл, проверяет архивацию,
        добавляет UID (если надо) и полностью перерисовывает GUI.
        """
        self._clear_tasks_layout()
        
        data = self.read_tasks_from_file()
        self.group_by_period = data.get("group_by_period", True)
        
        today_str = QtCore.QDate.currentDate().toString("yyyy-MM-dd")
        tasks_modified = False 
        
        active_tasks_to_draw = []
        ...
        for task in data.get("tasks", []):
            ...
            if not task.get("archive", False):
                active_tasks_to_draw.append(task)
        
        for task_data in reversed(active_tasks_to_draw):
    ...
    def update_task_in_file(self, uid, key, value):
        """Находит таск по UID, меняет поле и перезаписывает файл.
        НЕ ПЕРЕРИСОВЫВАЕТ GUI."""
        if not uid:
            return False
        ...
        for task in data.get("tasks", []):
            if task.get("uid") == uid:
                task[key] = value
                if key == "checked" and value is True:
                    task["date"] = QtCore.QDate.currentDate().toString("yyyy-MM-dd")
                task_found = True
                break
        
        if task_found:
            success = self.write_data_to_file(data)
            if success:
                self.on_task_edited()
            return success
        else:
            print(f"Error: Could not find task with UID {uid} to update.")
            return False
            
    def delete_task_from_file(self, uid):
        """Находит таск по UID, удаляет его и перезаписывает файл."""
        if not uid:
            return
        ...
        new_tasks_list = [
            task for task in data.get("tasks", []) if task.get("uid") != uid
        ]
        
        if len(new_tasks_list) < len(data["tasks"]):
            data["tasks"] = new_tasks_list
            self.write_data_to_file(data)
            self.load_and_redraw_all() # Удаление требует перерисовки
        else:
            print(f"Error: Could not find task with UID {uid} to delete.")
    ...
    def add_task(self, task_data):
        widget = TaskWidget(task_data, self.font, self) 
    ...
    def handle_new_task(self):
        ...
        if text:
            ...
            data.get("tasks", []).insert(0, task_data)
            self.write_data_to_file(data)
            self.new_task.clear()
            self.load_and_redraw_all()
    ...
    def get_archived_tasks(self):
        data = self.read_tasks_from_file()
        return [t for t in data["tasks"] if t.get("archive", False)]
    ...
    def toggle_group_by_period(self):
        data = self.read_tasks_from_file()
        data["group_by_period"] = not data.get("group_by_period", True)
        self.write_data_to_file(data)
...
if __name__ == "__main__":
    ...
    # VVVVVV (ИЗМЕНЕНИЕ 3: Устанавливаем AppName) VVVVVV
    # Это нужно, чтобы QStandardPaths знал, 
    # в какую папку (AppData/Roaming/TrayTodo) сохранять
    app.setOrganizationName("YourCompany") 
    app.setApplicationName("TrayTodo")
    # ^^^^^^^^ КОНЕЦ ИЗМЕНЕНИЯ 3 ^^^^^^^^
