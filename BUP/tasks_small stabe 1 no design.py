...
class ArchiveWindow(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # VVVVVV (ИЗМЕНЕНИЕ: Флаг .Tool ВОЗВРАЩЕН) VVVVVV
        ...
        # Отступ 5 (остался)
        self.list_widget.setIndentation(5) 
    ...
    def style_header_item(self, item):
        ...
        font.setBold(True)
        item.setFont(0, font)
        item.setForeground(0, QtGui.QColor("#555"))
        item.setExpanded(True) # По умолчанию разворачиваем
    ...
    def load_archive(self):
        ...
        for task in archived_tasks:
            ...
            task_item.setData(0, QtCore.Qt.UserRole, task_id) # ID для удаления
    ...
    # VVVVVV (ИЗМЕНЕНИЕ: Логика выравнивания по НИЖНЕМУ краю) VVVVVV
    def show_and_position(self):
        """Показывает окно СЛЕВА от главного, ВЫРАВНИВАЯ ПО НИЖНЕМУ КРАЮ"""
        ...
        margin = 10
        # X = X_главного - ширина_архива - отступ
        new_x = main_x - self.width() - margin
        
        # Y = Y_главного + Высота_главного - Высота_архива
...
class SimpleTodo(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # Заголовок с датой (остался)
        now = datetime.now()
        weekdays_ru = ["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"]
        months_ru_gen = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"]
        
        day_str = weekdays_ru[now.weekday()]
        month_str = months_ru_gen[now.month - 1]
        date_title = f"{day_str}, {now.day} {month_str}"

        self.title_label = QtWidgets.QLabel(date_title)
        self.title_label.setStyleSheet("""
            font-weight: normal; 
            color: #777; 
            padding-right: 2px;
        ...
        self.input_field.setPlaceholderText("Add task and press Enter")
    ...
    def load_tasks(self):
        """Читает файл, АВТО-АРХИВИРУЕТ СТАРОЕ и "рисует" QListWidget (ТОЛЬКО НЕ АРХИВНЫЕ)"""
        
        print("Loading tasks and checking for stale items...")
        
        today_obj = datetime.now().date()
        today_date = today_obj.isoformat()
        yesterday_date = (today_obj - timedelta(days=1)).isoformat()
        ...
        for task in tasks_list:
            ...
            is_archived = task.get("archive", False)
            
            if task_date and (task_date != today_date) and is_checked and not is_archived:
                task["archive"] = True
                task["date"] = yesterday_date # Ставим вчерашнюю дату
                tasks_changed = True
                print(f" -> Auto-archiving stale task {task.get('id')[:4]}...")

        if tasks_changed:
            print("Saving auto-archived tasks...")
        ...
        for task in active_tasks: # Используем отфильтрованный список
    ...
    def add_task(self):
        ...
        new_task = {
            "id": str(uuid.uuid4()),
            "text": text,
            "date": datetime.now().date().isoformat(), # Только дата
            "checked": False,
            "archive": False,
            "important": False 
        }
    ...
    def on_item_changed(self, item):
        """Срабатывает, когда мы ставим/убираем галочку"""
        
        task_id = item.data(QtCore.Qt.UserRole)
        is_checked = item.checkState() == QtCore.Qt.CheckState.Checked
        
        if not task_id:
            print("Error: item changed, but has no ID.")
            return

        print(f"Updating task ID {task_id[:4]}... -> checked={is_checked}")
        ...
        if task_found:
            ...
            print(f"Error: Could not find task with ID {task_id} to update.")
    ...
    def show_main_list_menu(self, position):
        ...
        important_action = menu.addAction("Important") # БЕЗ ИКОНКИ
        important_action.setCheckable(True)
        important_action.setChecked(is_important)
        
        menu.addSeparator()

        delete_action = menu.addAction("Delete") # БЕЗ ИКОНКИ
    ...
    def move_checked_to_archive(self):
        ...
        for task in tasks_list:
            if task.get("checked") and not task.get("archive", False):
                task["archive"] = True
                task["date"] = today_date_iso # Ставим сегодняшнюю дату
                tasks_changed = True
